...
def serve_xgboost_roc_curve(model, X_test, y_test):
    decision_test = model.predict_proba(X_test)[:, 1] - model.predict_proba(X_test)[:, 0]
...
def serve_xgboost_pie_confusion_matrix(model, X_test, y_test, Z, threshold):
    # Compute threshold
    scaled_threshold = threshold * (Z.max() - Z.min()) + Z.min()
    y_pred_test = (model.predict_proba(X_test)[:, 1] - model.predict_proba(X_test)[:, 0] > scaled_threshold).astype(int)
